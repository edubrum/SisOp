...
//  cc -pthread peterson_spinlock-Vatom.c -o petAt
...
volatile int flag[2];                // variaveis do algoritmo de peterson
volatile int turn;
sem_t mutex;   
...
{	
	...
	sem_init (&mutex , 0, 1);  // para tornar atribuicao uma operacao atomica	############################
}


void atomicTurnAssignment(int t){  // atribuicao atomica somente ###########################################
                                   //  ISTO É UMA CAIXA PRETA AGORA - VAMOS APRENDER SEMAFOROS DEPOIS 
                                   //  AGORA CONSIDERE APENAS QUE A ATRIBUICAO DE turn É ATOMICA 
    sem_wait(&mutex);	     
	turn = t;                // mas antes daa aa outra trhead a chance de adquirir o lock 
    sem_post(&mutex);	
} 
...
void lock(int self)         // executado antes da secao critica
{
    ...
    atomicTurnAssignment(1-self);  // mas antes daa aa outra trhead a chance de adquirir o lock ##############
...
volatile int ans = 0;                // a variavel compartilhada!!   a ser protegida
 
void* func(void *s)         // uma funcao simples executada em duas threads criadas no main
{
    int i = 0;
    int self = (int *)s;
